...
class Board:
    def __init__(self):
        self.squares = {} #A dictionary where the keys are the name of the squares and the values are the tile located in the square, for example ("a1", "s")
        self.border_squares = set(["h8"]) #A set with the names of the squares that has no tile, but are adjacent to squares with tiles placed
        for row in range(1, 16):
            for column in string.ascii_lowercase[:15]:
                self.squares[column + str(row)] = None
        self.plays = []
        self.border_squares_history = [["h8"]] #stores border_squares at each moment for comodity and speed
        ...
        for i in range(len(squares_used) - 1):
            ...
            while square != expected_square:
                if (square is None) or (self.squares[square] is None):
                    ...
                    return False
                square = move(square)
        return any((sqr in self.border_squares) for sqr in squares_used) #Finally, if a border square is used, the word is connected
    
    def __get_all_new_words(self, play : Dict[str, str], direction : str, check_for_real_words : str) -> List[Tuple[str, str, int]]:
        ...
        """
        words = [] #List of tuples. Each tuple describes one of the new words formed: (start_position, word, points). For example ("f3", "casa", "6")
        ...
        for square in play:
            if (self.squares[move(square)] is not None) or (self.squares[move_back(square)] is not None):
                words.append(self.__get_word({square : play[square]}, direction, check_for_real_words))
        return words
    
    def __get_word(self, play : Dict[str, str], direction : str, check_for_real_words : str) -> Tuple[str, str, int]:
        ...
        while (square is not None) and (tile is not None):
            word = word + tile
            sqr_word_multiplicator, sqr_tile_multiplicator = (square_multiplicator(square)) if square in play else (1, 1)
    ...
    def __update(self, play : Dict[str, str]) -> None:
        ...
        for square in play:
            ...
            neighbors = [Board.up_a_square(square), Board.down_a_square(square), Board.left_a_square(square), Board.right_a_square(square)]
    ...
    def __sort_squares_used(squares : List[str], direction : str) -> List[str]:
        ...
        sorting_criteria = (lambda sqr : sqr[0]) if (direction == "horizontal") else (lambda sqr : int(sqr[1:]))
